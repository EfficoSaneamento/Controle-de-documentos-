...
const MAX_ANEXOS_EMAIL_BYTES = 16 * 1024 * 1024;
...
function doPost(e) {
  ...
  try {
    ...
    mensagem = error && error.message ? error.message : "Falha ao processar o envio.";
...
function validarFotos(fotos) {
  ...
  Object.keys(fotos).forEach(function (key) {
    ...
    totalBytes += Math.floor((valor.split(",")[1].length * 3) / 4);
    if (totalBytes > MAX_ANEXOS_EMAIL_BYTES) {
      throw new Error("O tamanho total dos anexos excede o limite de 16 MB.");
    }
  ...
  if (fotos) {
    for (const key in fotos) {
      ...
      if (
        ...
        checklistHtml += `
          <li>
            ❌ <b>${escaparHtml(key.replace(/_/g, " "))}</b>: Não informado
          </li>
        `;
        continue;
      }
